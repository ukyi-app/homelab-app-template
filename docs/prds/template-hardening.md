...
|----|------|--------|-------|
| F-8 | ✅ **처리됨(2026-07-13, PR #20)** — template-ci(`scaffold-args`)에 **`ownString` 본문 정합 가드**를 배선했다. 읽기 규율(`ownString` — own 데이터 서술자만 채택, accessor·Proxy 트랩·문자열화 훅 미호출)은 worker(`src/index.ts`)·api(`src/db.ts`)에 **두 벌** 복제돼 있고(아키타입 간 공유 모듈 금지 = 의도, 앱엔 하나만 복사), F-7에서 실제로 한쪽만 굳어 **503 본문** 유출 구멍이 열렸다. 가드는 `scaffold/archetypes/*/src/*.ts`에서 정의를 **발견**(열거 아님 — 새 아키타입이 DB를 얻어 복제하면 자동 편입)해 각 본문을 추출(CRLF 선제거·`export` 정규화)·해시하고 **바이트 동일성**을 강제한다. **왜 표 대조가 아니라 본문 동일성인가**: api는 sink가 둘(로그+`/readyz` 503 본문)이라 테스트 표가 정당하게 다르다(describe 이름부터) — 케이스 집합 대조는 그 차이를 오탐하거나 헐거워져 뮤테이션 RED를 못 박는다(이 레포가 받지 않는 가드). 대신 '표가 겨누는 코드'를 하나로 묶으면 worker의 포괄 표(25행)가 양쪽을 보증한다(한쪽만 바꾸면 가드가, 양쪽 동일 약화는 그 표가 bun test에서 RED — 상보적). **적대적 리뷰(11 에이전트·3 렌즈) 반영**: 발화 불가한 `n_hash` 핀(주석이 "삼킴 병리를 개수로 잡는다"고 과장)을 실효 있는 **추출 무결성 검사**(화살표블록 1개 + 마지막 줄 `};` — overrun을 '갈라졌다'로 오진 않고 '추출이 어긋났다'로 지목)로 교체 + CRLF 선제거. 뮤테이션 RED 5종 실측(verification.md §15) | F-7 code-review | PR #20 |
| F-1 | ✅ **처리됨(2026-07-13)** — autodiscover + `requireConfig=required`로 전환해 '설정 파일의 존재 = opt-in'이 되게 했다(신규 앱은 템플릿이 설정을 동봉하므로 자동 편입, 비-앱 레포는 `disabled-no-config`로 자동 스킵 — dry-run 실측). PR: homelab#347, page#9, trip-mate-api#30. **owner 수동 잔여도 해소(2026-07-14 검증)**: writer App(ukyi-homelab-writer, install id 140005764) 설치 범위가 조직 전체(`repository_selection: all`, `updated_at` 2026-07-13)로 확대돼 있어 autodiscover가 org 전 레포(현재 6개, 신규 앱 포함)를 자동 편입한다 — fail-safe 제약 없음 | P-3 | homelab#347 |
...
| F-7 | ✅ **처리됨(2026-07-13, PR #19)** — worker에 엔트리 가드(`import.meta.main`) + `createWorker(work)` 팩토리 + 테스트 9개(타입 표 25행)를 넣고 Dockerfile 게이트에 `RUN bun run test`를 배선(I-5의 드리프트 가드가 셋의 정합을 강제). **리뷰가 살아남는 뮤턴트를 찾았다**: `ownString`의 `typeof d.value === "string"`을 지워도 테스트가 전부 초록이었다 — own **데이터** 프로퍼티인데 값이 객체인 경우(`{message:{toString:()=>password}}`)가 표에 없었고, `errLine`의 보간이 그 훅을 부른다(tsc도 못 잡는다 — `PropertyDescriptor.value`가 `any`라 반환 타입이 거짓말이다). **★같은 구멍이 api에도 있었고 더 나빴다** — 유출이 **HTTP 503 본문**으로 나간다(파드에 닿는 누구나 읽는다). 게다가 `Object.prototype` 오염 테스트가 worker엔 있는데 **api엔 없었다**. 둘 다 닫았다(api 27 → 29). template-ci에 `timeout-minutes: 10`(엔트리 가드가 빠지면 `bun test`가 hang한다) | I-3 code-review | PR #19 |
...
| F-3 | ✅ **처리됨(2026-07-13, 업스트림 리포트)** — 두 건 다 pg-free **최소 재현**으로 좁혀 Node 대조와 함께 제출했다. **①** "Bun이 pg pool의 unhandled error를 삼킨다"의 근본원인을 격리: Bun은 소켓 `'data'` 리스너에서 **던져진 예외를, 소켓에 `'error'` 리스너가 있으면 그 리스너로 오배송**한다(uncaughtException 대신) — pg가 소켓 error 핸들러(`reportStreamError`)를 달아둬서 unhandled pool error가 사라졌던 것이다(리스너 없으면 Bun도 정상 전파). Node 크래시(exit 1)/Bun 삼킴(exit 0), 결정적. 제출([#34064](https://github.com/oven-sh/bun/issues/34064)) 직후 Bun 봇이 **재현 확인 + 수정 PR([#34066](https://github.com/oven-sh/bun/pull/34066)) 착수**. dedup 봇이 더 오래된 열린 [#29761](https://github.com/oven-sh/bun/issues/29761)(같은 근본원)을 지적해 그쪽으로 **통합**했다 — Node 대조·오배송 메커니즘·pg password 보안 사례를 [#29761 코멘트](https://github.com/oven-sh/bun/issues/29761#issuecomment-4965206698)로 옮기고 #34064는 중복 close. **②** "인스펙터가 password 노출"의 실제 메커니즘: pg는 password를 **non-enumerable**로 둬 로그에서 숨기는데(실측: `getOwnPropertyDescriptor(client,'password').enumerable===false`) Bun 콘솔은 non-enumerable을 찍는다(Node는 `util.inspect` 기본 `showHidden:false`로 숨김). `console.error(err)` 실측 Node 0/Bun 3회 유출. 이미 열린 이슈([oven-sh/bun#12361](https://github.com/oven-sh/bun/issues/12361))라 신규 대신 **pg password 보안 사례를 코멘트**로 추가([#issuecomment](https://github.com/oven-sh/bun/issues/12361#issuecomment-4957012393)). 재현본: scratch `f3-repro/`(레포 커밋 아님) | I-3 실행 중 발견 | bun#29761·#34066, bun#12361 |
