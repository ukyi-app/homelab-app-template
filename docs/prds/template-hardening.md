...
| F-3 | ✅ **처리됨(2026-07-13, 업스트림 리포트)** — 두 건 다 pg-free **최소 재현**으로 좁혀 Node 대조와 함께 제출했다. **①** "Bun이 pg pool의 unhandled error를 삼킨다"의 근본원인을 격리: Bun은 소켓 `'data'` 리스너에서 **던져진 예외를, 소켓에 `'error'` 리스너가 있으면 그 리스너로 오배송**한다(uncaughtException 대신) — pg가 소켓 error 핸들러(`reportStreamError`)를 달아둬서 unhandled pool error가 사라졌던 것이다(리스너 없으면 Bun도 정상 전파). Node 크래시(exit 1)/Bun 삼킴(exit 0), 결정적. 열린 중복 없어 **신규 제출**: [oven-sh/bun#34064](https://github.com/oven-sh/bun/issues/34064). **②** "인스펙터가 password 노출"의 실제 메커니즘: pg는 password를 **non-enumerable**로 둬 로그에서 숨기는데(실측: `getOwnPropertyDescriptor(client,'password').enumerable===false`) Bun 콘솔은 non-enumerable을 찍는다(Node는 `util.inspect` 기본 `showHidden:false`로 숨김). `console.error(err)` 실측 Node 0/Bun 3회 유출. 이미 열린 이슈([oven-sh/bun#12361](https://github.com/oven-sh/bun/issues/12361))라 신규 대신 **pg password 보안 사례를 코멘트**로 추가([#issuecomment](https://github.com/oven-sh/bun/issues/12361#issuecomment-4957012393)). 재현본: scratch `f3-repro/`(레포 커밋 아님) | I-3 실행 중 발견 | bun#34064, bun#12361 |
