...
|----|---------|----------|----------|--------|--------|
| F-8 | **복제된 규율의 테스트 표를 동기화할 장치가 없다** — 에러 포매터의 읽기 규율(`ownString`/`errLine`)이 worker와 api에 **두 벌** 존재한다(아키타입 간 공유 모듈 금지는 의도 — 앱엔 하나만 복사된다). 그런데 두 테스트 표가 어긋나도 잡는 게 없다: F-7에서 `Object.prototype` 오염 테스트가 worker엔 있고 api엔 없어 **실제로 구멍이 하나 더 열려 있었다**(HTTP 503 본문 유출). 두 파일의 `ownString` 본문 동일성 또는 표의 케이스 이름 집합을 template-ci에서 대조하는 가드를 검토할 만하다 | F-7 code-review | 별도 판단 |
