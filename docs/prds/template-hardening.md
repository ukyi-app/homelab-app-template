...
|----|------|--------|-------|
| F-8 | ✅ **처리됨(2026-07-13, 이 PR)** — template-ci(`scaffold-args`)에 **`ownString` 본문 정합 가드**를 배선했다. 읽기 규율(`ownString` — own 데이터 서술자만 채택, accessor·Proxy 트랩·문자열화 훅 미호출)은 worker(`src/index.ts`)·api(`src/db.ts`)에 **두 벌** 복제돼 있고(아키타입 간 공유 모듈 금지 = 의도, 앱엔 하나만 복사), F-7에서 실제로 한쪽만 굳어 **503 본문** 유출 구멍이 열렸다. 가드는 `scaffold/archetypes/*/src/*.ts`에서 정의를 **발견**(열거 아님 — 새 아키타입이 DB를 얻어 복제하면 자동 편입)해 각 본문을 추출(CRLF 선제거·`export` 정규화)·해시하고 **바이트 동일성**을 강제한다. **왜 표 대조가 아니라 본문 동일성인가**: api는 sink가 둘(로그+`/readyz` 503 본문)이라 테스트 표가 정당하게 다르다(describe 이름부터) — 케이스 집합 대조는 그 차이를 오탐하거나 헐거워져 뮤테이션 RED를 못 박는다(이 레포가 받지 않는 가드). 대신 '표가 겨누는 코드'를 하나로 묶으면 worker의 포괄 표(25행)가 양쪽을 보증한다(한쪽만 바꾸면 가드가, 양쪽 동일 약화는 그 표가 bun test에서 RED — 상보적). **적대적 리뷰(11 에이전트·3 렌즈) 반영**: 발화 불가한 `n_hash` 핀(주석이 "삼킴 병리를 개수로 잡는다"고 과장)을 실효 있는 **추출 무결성 검사**(화살표블록 1개 + 마지막 줄 `};` — overrun을 '갈라졌다'로 오진 않고 '추출이 어긋났다'로 지목)로 교체 + CRLF 선제거. 뮤테이션 RED 5종 실측(verification.md §15) | F-7 code-review | 이 PR |
