...
2. `.app-config.yml` 수정 (kind/resources/route/secrets — 모든 설정·비밀은 secrets로 봉인)
...
## 비밀 값
앱 레포 코드에는 절대 넣지 않는다. `.app-config.yml`의 `secrets:`에 이름만 선언하고,
값은 `pnpm secret:seal`로 봉인한다(`.env` → `<앱>-secrets.sealed.yaml`, SealedSecret).
owner의 create-app/update-secrets가 이 봉인 파일을 검증·배선한다.
