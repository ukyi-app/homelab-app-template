...
const NO_DB_ENV = [
  ...
  "# 프로덕션 URL은 homelab create-database가 conn SealedSecret으로 만들어 파드에 직접 주입하고(키 이름은 앱 이름이",
  "# 아니라 DB 이름에서 유도된다), 로컬 URL은 규약 2대로 .env.local에 둔다.",
];
...
const DOC: Record<Arch, { runtime: string; gates: string[]; cmds: string[]; note: string[]; secret: string; env: string[] }> = {
  ...
  api: {
    ...
    note: [
      ...
      "DB URL은 접두사를 추측하지 않고 env 접미사(`_DATABASE_URL`)로 자동 발견한다(`src/db.ts`) — 접두사는 앱 이름이 아니라 **DB 이름**에서 나오므로 앱이 그걸 알 필요가 없게 만든 것이다. 규약은 `.env.example` 주석에.",
    ],
    secret: "DB 접속 URL은 `.env`에 넣지 않는다 — `.env`는 통째로 봉인되어 프로덕션까지 따라가는데, 거기엔 플랫폼이 주입한 URL이 이미 있다. `src/db.ts`는 플랫폼의 접두사 키(`<DB>_DATABASE_URL`)를 우선하고 generic `DATABASE_URL`은 그게 없을 때의 fallback으로만 쓰므로, 봉인한 generic 값은 프로덕션에서 조용히 무시된다. 접두사 키를 직접 봉인했는데 플랫폼도 같은 앱에 DB를 주입하면 후보가 둘이 되어 앱이 기동에서 죽는다(`secret:seal`이 DB URL 키를 만나면 경고한다). 프로덕션 URL은 homelab이 별도 SealedSecret으로 주입하고, 로컬 URL은 `.env.local`에 둔다 — 규약은 `.env.example` 주석에.",
    env: [
      "# --- 규약 3) DB 접속 URL은 .env에 넣지 않는다 ---",
      "# 플랫폼이 스스로 파드에 주입하는 값이라, .env에 적어 봉인하면 프로덕션에서 그 주입값에 지거나 충돌한다",
      "# (secret:seal이 DB URL 키를 만나면 경고한다).",
      "#",
      "# 프로덕션: homelab create-database(provision-db)가 conn SealedSecret을 따로 만들어 파드에 주입한다.",
      "#   <DB>_DATABASE_URL          런타임 — owner 롤, 풀러(PgBouncer) 경유",
      "#   <DB>_MIGRATE_DATABASE_URL  마이그레이션 — 같은 owner 롤이지만 직결(session 시맨틱이 필요해서)",
      "#   <DB>_RO_DATABASE_URL       읽기 전용 롤, 직결 — 디버깅용(앱에 배선하지 않는다)",
      "#   런타임이 _MIGRATE_를 잡으면 롤이 아니라 '풀러'가 빠진다 — 앱 풀이 공유 클러스터에 직결로 붙어",
      "#   max_connections를 고갈시킨다. 그래서 src/db.ts는 _MIGRATE_/_RO_를 런타임 후보에서 제외한다.",
      "# 로컬: homelab의 db-url 도구가 같은 키 이름으로 .env.local에 URL을 쓴다(규약 2 — 봉인 대상이 아니다).",
      "#",
      "# ★<DB>는 '앱 이름'이 아니다 — create-database에 준 *DB 이름*의 UPPER_SNAKE다",
      "#   (provision-db.ts: name.replaceAll(\"-\",\"_\").toUpperCase()). DB 이름은 홈랩 전역에서 유일하기만 하면",
      "#   되고 앱 이름과 같을 의무가 없다: 실제로 앱 trip-mate-api는 DB trip-mate를 받아 키가 TRIP_MATE_DATABASE_URL이다.",
      "#   내 앱의 키 이름은 추측하지 말고 확인한다 —",
      "#     · homelab apps/<app>/deploy/prod/values.yaml의 envFrom secretRef 이름: db-<DB이름>-conn",
      "#     · create-database --dry-run 출력의 envKeys",
      "#     · 파드에 실제로 주입된 env 키",
      "#   앱 코드는 이걸 몰라도 된다 — src/db.ts가 접두사가 아니라 접미사 _DATABASE_URL로 찾기 때문이다",
      "#   (_MIGRATE_/_RO_ 키는 런타임 후보에서 제외). 접두사를 알아야 하는 건 '직접 봉인할 때'뿐이고, 그때 틀리면 아래가 문다.",
      "# ★우선순위: 플랫폼이 주입한 접두사 키가 이긴다. 접두사 없는 generic DATABASE_URL은 접두사 키가 하나도 없을",
      "#   때만 쓰는 fallback이다(그 이름은 homelab dev.ts의 로컬 Postgres용이다). 그래서 DB URL을 .env에 적어",
      "#   봉인해도 프로덕션에선 플랫폼 값에 져서 조용히 무시된다 — 로컬 개발 URL은 .env가 아니라 .env.local에.",
      "# ★접두사 키가 둘 이상 보이면 src/db.ts는 기동에서 죽는다(CrashLoop) — 어느 DB가 이 앱 것인지 정할 수 없고,",
      "#   조용히 하나를 고르면 엉뚱한 DB에 쓰고도 초록이다(되돌릴 수 없다). 봉인한 DB URL 키와 플랫폼 주입 키가",
      "#   겹치는 게 대표적인 경우다. 소스를 하나로 줄이거나, 그럴 수 없으면 자동 발견을 포기하고 풀을 직접 만들어",
      "#   createApp(pool)로 넘긴다(src/db.ts의 발견 규칙을 통째로 우회한다).",
    ],
  },
  site: {
