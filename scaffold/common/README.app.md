...
2. **봉인** — `bun run secret:seal` (로컬에 `kubeseal` CLI 필요).
   ...
   `deploy/{{DEPLOY}}-secrets.sealed.yaml`에 쓴다. 평문 Secret은 디스크를 거치지 않고 kubeseal stdin으로만
   흐른다. `bun run secret:seal --dry-run`은 봉인 없이 대상 키 목록만 보여준다(값 미포함).
3. **커밋** — `deploy/{{DEPLOY}}-secrets.sealed.yaml`을 커밋·push한다. 클러스터의 sealed-secrets
...
- 런타임: {{RUNTIME}}
- 배포 식별자(`{{DEPLOY}}`) = GitHub 레포명 — 배포의 단일 원천이다. `create-app`이 `repoName === app`을 강제하고,
  `secret:seal`의 `--app` 값과 봉인 파일명(`deploy/{{DEPLOY}}-secrets.sealed.yaml`)이 여기서 나온다.
- 앱 이름(`{{APP}}`) = `package.json`의 `name`·이 문서의 제목(표시용) — 규약상 레포명과 같게 둔다.
- 레포를 rename하면 `secret:seal`의 `--app` 값과 `deploy/{{DEPLOY}}-secrets.sealed.yaml` 파일명을 새 레포명으로
  고치고 다시 봉인한다(봉인 이름이 `<레포명>-secrets`가 아니면 `create-app`이 거부한다). `package.json`의 `name`도 함께 맞춘다.
